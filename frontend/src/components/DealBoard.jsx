...
const ACCENT = {
  ...
  cyan: { dot: 'bg-cyan-400', border: 'border-t-cyan-400', chip: 'text-cyan-300 bg-cyan-400/10' },
    ...
    <div className={`rounded-lg border border-surface-border border-t-2 ${ACCENT[accent].border} bg-surface shadow-sm`}>
      ...
      >
        <div className="mb-2 flex items-start justify-between gap-2">
          <span className="truncate text-sm font-medium text-slate-100" title={deal.name ?? ''}>
            {deal.name || 'Untitled deal'}
          </span>
          {amount != null && <span className="shrink-0 text-sm font-semibold text-slate-100">{amount}</span>}
        </div>
        <p className="mb-1 truncate text-xs text-slate-400" title={deal.company ?? ''}>
      ...
      {expanded && (
        <div className="border-t border-surface-border px-3 pb-3 pt-2">
          {items.length === 0 ? (
            <p className="px-1 py-2 text-xs text-slate-500">No line items on this deal.</p>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-sm text-slate-100" title={item.name ?? ''}>
                        {item.name || 'No description'}
                      </p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className="text-[10px] uppercase tracking-wide text-slate-500">Qty</p>
                      <p className="tabular-nums text-sm font-medium text-slate-100">{item.quantity ?? '—'}</p>
                    </div>
                  </div>
                </li>
              ))}
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-surface-border bg-surface-raised p-4">
        ...
        <label className="flex min-w-48 flex-col gap-1.5">
          <span className="text-xs font-medium uppercase tracking-wide text-slate-500">Pipeline</span>
          <select
              ...
              >
                <div className="border-b border-surface-border px-4 py-3">
                  <div className="mb-1 flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-slate-100">{stage.key}</span>
                    <span className={`ml-auto rounded-full px-2 py-0.5 text-xs font-medium ${ACCENT[accent].chip}`}>
                      {stage.deals.length}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500">{subtotal ?? '$0'} total</p>
                </div>
