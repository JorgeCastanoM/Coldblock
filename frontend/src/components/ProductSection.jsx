...
function CostCell({ amount, currency, emphasis }) {
  if (amount == null) return <span className="text-slate-600">—</span>
  ...
  return (
    <span className="inline-flex items-baseline gap-1.5">
      <span className={emphasis ? 'text-slate-100' : 'text-slate-300'}>${formatted}</span>
      <span className="rounded bg-surface px-1 py-0.5 text-[10px] font-medium uppercase tracking-wide text-slate-500">
        {currency || 'CAD'}
      </span>
    </span>
  )
...
const columns = [
  {
    key: 'sku',
    label: 'SKU',
    cellClassName: 'font-mono text-[13px] font-medium text-slate-100',
  },
  {
    key: 'description',
    label: 'Description',
      ...
      <details className="group rounded-xl border border-surface-border bg-surface-raised open:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.03)]">
        <summary className="flex cursor-pointer list-none select-none flex-wrap items-center justify-between gap-x-4 gap-y-3 px-5 py-4 marker:content-none [&::-webkit-details-marker]:hidden">
          <div className="flex min-w-0 items-center gap-3">
            <span
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-surface-border bg-surface text-slate-500 transition group-open:rotate-90 group-open:border-sky-500/30 group-open:text-sky-300"
              aria-hidden="true"
            >
              ▸
            </span>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2.5">
                <h2 className="text-base font-semibold text-slate-100 sm:text-lg">{name}</h2>
                {showFinishedStock && <StockBadge qty={finishedQtyOnHand} sku={finishedSku} />}
              </div>
              {showFinishedStock && (
                <p className="mt-0.5 font-mono text-[11px] text-slate-500">{finishedSku}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 pl-10 sm:pl-0">
            {showFinishedStock && (
              <button
                ...
                className="rounded-lg border border-surface-border bg-surface px-3 py-1.5 text-xs font-medium text-slate-200 transition hover:border-sky-500/40 hover:text-sky-300"
              >
                Serials
              </button>
            )}
            {buildCostLabel && (
              <div className="text-right text-sm">
                <p className="font-semibold tabular-nums text-slate-100">{buildCostLabel}</p>
            ...
            <span className="rounded-full bg-surface px-3 py-1 text-xs font-medium text-slate-400 ring-1 ring-inset ring-surface-border">
